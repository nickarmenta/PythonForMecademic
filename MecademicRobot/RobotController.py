...
class RobotController:
    ...
    def __init__(self, address, firmware_version):
        ...
        self.answer_dict = {'ActivateRobot': [2000,2001], 'ActivateSim': [2045], 'ClearMotion': [2044],
                            'DeactivateRobot': [2004], 'BrakesOn': [2010], 'BrakesOff': [2008], 
                            'GetConf': [2029], 'GetJoints': [2026], 'GetStatusRobot': [2007], 
                            'GetStatusGripper': [2079], 'GetPose': [2027], 'Home': [2002,2003], 
                            # May need to append 3004 instead of just forcing it
                            'PauseMotion': [2042,3004], 'ResetError': [2005,2006], 'ResumeMotion': [2043], 
                            'SetEOB': [2054,2055], 'SetEOM': [2052,2053]}
    ...
    def exchange_msg(self, cmd, delay=20, decode=True):
        ...
        if(not self.error):                                 #if there is no error
            ...
            if status:                              #if the command was sent
    ...
    def get_data(self, delay=0.1):
        ...
        """
        if self.feedbackSocket is None:                         #check that the connection is established
            return                                      #if no connection, nothing to receive
        self.feedbackSocket.settimeout(delay)                   #set read timeout to desired delay
        try:
            raw_msg = self.feedbackSocket.recv(256).decode('ascii')         #read message from robot
    ...
    def _get_robot_status(self, response):
        ...
        for resp_code in code:
            if response.find(resp_code) != -1:
                self.robot_status = self._decode_fdbk_msg(response, resp_code)
    ...
    def _get_joints(self, response):
        ...
        for resp_code in code:
            if response.find(resp_code) != -1:
                self.joints = self._decode_fdbk_msg(response, resp_code)
    ...
    def _get_cartesian(self, response):
        ...
        for resp_code in code:
            if response.find(resp_code) != -1:
                self.cartesian = self._decode_fdbk_msg(response,resp_code)
    ...
    def _get_joints_vel(self, response):
        ...
        for resp_code in code:
            if response.find(resp_code) != -1:
                self.joints_vel = self._decode_fdbk_msg(response,resp_code)
    ...
    def _get_torque_ratio(self, response):
        ...
        for resp_code in code:
            if response.find(resp_code) != -1:
                self.torque = self._decode_fdbk_msg(response,resp_code)
    ...
    def _get_accelerometer(self,response):
        ...
        for resp_code in code:
            if response.find(resp_code) != -1:
                self.accelerometer = self._decode_fdbk_msg(response,resp_code)
    ...
    def _decode_fdbk_msg(self, response, resp_code):
        """
